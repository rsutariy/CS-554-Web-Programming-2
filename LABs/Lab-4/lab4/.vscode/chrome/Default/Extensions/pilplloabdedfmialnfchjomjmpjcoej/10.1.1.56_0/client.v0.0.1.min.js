...
/***/ function(module, exports, __webpack_require__) {
	...
	var hashObject = (0, _Utils.buildObjectFromStringWithDelimiters)(window.location.hash, '-', '_', '#');
	...
	conf.load().then(function () {
	    ...
	    if (env_ && clientsEnvsConfiguration[env_]) {
	        ...
	        for (var i = 0; !loadedApp && i < extensionComponentKeys.length; i++) {
	            ...
	            if (component.type === envConfig.parentType) {
	                if (envConfig.bodyStyling && component.style) {
	                    try {
	                ...
	                if (envConfig.rootStyling && component.rootStyle) {
	                    try {
	    ...
	    if (loadedApp) {
	        var event = envConfig.finishEvent ? envConfig.finishEvent : _Consts.CLIENT_LOADED_EVENT;
	        _EventsService2.default.trigger(event);
	        _LoggerService2.default.debug('event: ' + event);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _Clock = __webpack_require__(83);

	var _Clock2 = _interopRequireDefault(_Clock);

	var _Weather = __webpack_require__(84);

	var _Weather2 = _interopRequireDefault(_Weather);

	var _RssFeed = __webpack_require__(85);

	var _RssFeed2 = _interopRequireDefault(_RssFeed);

	var _RssFeedsContainer = __webpack_require__(86);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _fetchJsonp = __webpack_require__(35);
	...
	function actionCreator(behaviour) {
	    ...
	    switch (behaviour.do) {
	        case ActionTypes.REDIRECT_CURRENT_TAB:
	            retFunc = function retFunc(args) {
	                ...
	                if (behaviour.url && behaviour.url != 'disable' && (cond === null || cond && cond.check(args))) {
	                    _ChromeService2.default.updateTab(_ChromeService2.default.getCurrentTabId(), behaviour.url);
	                }
	            };
	            break;
	        case ActionTypes.GET_SEARCH_SUGGESTIONS:
	            retFunc = function retFunc(args) {
	                var suggestUrl = self.search.getSuggestUrl(self.search.getEngineIdByIndex(self.search.getTempOrCurrentSearchEngineIndex()));
	                var cond = behaviour.c ? new _Condition2.default(behaviour.c) : null;
	                if (args.text === "") {
	                    self.setState({ rows: [] });
	                    ...
	                    if ((0, _Utils.strStartsWith)(window.location.href, "http://")) {
	                        (0, _fetchJsonp2.default)(url).then(function (response) {
	                            ...
	                            _LoggerService2.default.debug('parsing failed', ex);
	                        });
	                    } else {
	                        _HttpService2.default.doGetJSON(url, undefined, undefined, {}).then(function (json) {
	                            if (json[0].indexOf(args.text.trim()) > -1) {
	                                // if (json[0] === args.text.trim()) {
	                                if (json[1].length > 5) {
	                                    self.setState({ rows: json[1].slice(0, 5), selected: -1 });
	                                } else {
	        ...
	        case ActionTypes.HIDE_WHEN_CLICKED_OUTSIDE:
	            retFunc = function retFunc(args) {
	                if (selfIsMounted(self) && !ReactDOM.findDOMNode(self).contains(args.target)) {
	                    ...
	                    } else if (self.state && self.props.config.hasOwnProperty('hide')) {
	                ...
	                if ((cond === null || cond && cond.check(args)) && args.engineId) {
	                    ...
	                    if (search.isEnginePersist(args.engineId)) {
	                        if (args.engineId != pref.get(_Consts.SELECTED_SEARCH_ENGINE_KEY)) {
	                            ...
	                            _EventsService2.default.trigger(_Consts.SEARCH_ENGINE_CHANGED, [_assign(args)]);
	                            _LoggerService2.default.debug('event: ' + _Consts.SEARCH_ENGINE_CHANGED);
	                        }
	                    } else {
	                        if (args.engineId != pref.get(_Consts.TEMP_SEARCH_ENGINE_KEY)) {
	                            search.setCurrentSearchEngine(args.engineId);
	                            //ee.trigger(SEARCH_ENGINE_CHANGED);
	                            _EventsService2.default.trigger(_Consts.SEARCH_ENGINE_CHANGED, [_assign(args)]);
	                            _LoggerService2.default.debug('event: ' + _Consts.SEARCH_ENGINE_CHANGED);
	                        }
	                    }
	        ...
	        case ActionTypes.CHANGE_BACKGROUND:
	            retFunc = function retFunc(args) {
	                if (args.image) {
	                    if (behaviour.style && behaviour.style[args.image] != undefined) {
	                        self.state.style = behaviour.style[args.image];
	                    } else {
	                        self.state.style = '';
	                    }
	                    self.setState({
	        ...
	        case ActionTypes.CHANGE_COLOR_TO_GRAY:
	            retFunc = function retFunc(args) {
	                ...
	                } else if (args.image === "tile-bg.png") {
	        ...
	        case ActionTypes.SET_STATE:
	            retFunc = function retFunc(args) {
	                ...
	                } else if (behaviour.ignorable) {} else {
	                    var unselectedState = {};
	        ...
	        case ActionTypes.HIDE_OR_SHOW:
	            retFunc = function retFunc(args) {
	                ...
	                if (cond === null || cond && cond.check(args)) {
	                    ...
	                    if (behaviour.args != undefined) {
	                        args = _assign({}, args, behaviour.args);
	                    }
	                }
	        ...
	        case ActionTypes.TOGGLE_PREFERENCE:
	            retFunc = function retFunc(args) {
	                ...
	                if (cond === null || cond && cond.check(args)) {
	                    ...
	                    _LoggerService2.default.debug('TOGGLE_PREF_FINISHED');
	        ...
	        case ActionTypes.CHANGE_VISIBILITY:
	            retFunc = function retFunc(args) {
	                ...
	                _LoggerService2.default.debug("change_visibilty_finished");
	            };
	            break;
	        ...
	        case ActionTypes.UPDATE_WEATHER:
	            retFunc = function retFunc(args) {
	                try {
	                    (function () {
	                        var prefService = _ServiceFactory2.default.get(_Consts.PREFERENCES_SERVICE);
	                        var geodata_url = 'http://api.openweathermap.org/data/2.5/weather?appid=' + _Consts.WEATHER_API_KEY + '&units=' + (prefService.get(_Consts.UNITS_WEATHER_KEY) == '{{UNITS_WEATHER}}' ? 'metric' : prefService.get(_Consts.UNITS_WEATHER_KEY)) + '&lat={lat}&lon={lon}'.replace('{lat}', prefService.get(_Consts.GEO_DATA_KEY)[_Consts.LATITUDE_KEY]).replace('{lon}', prefService.get(_Consts.GEO_DATA_KEY)[_Consts.LONGITUDE_KEY]);
	                            ...
	                            if (behaviour.removeNewTab || behaviour.removeCurrentTab) {
	                                _ChromeService2.default.getLastFocusedWindow().then(function (windowDetails) {
	                                    _ChromeService2.default.getCurrentVisibleTab(windowDetails).then(function (tab) {
	                                        ...
	                                        tracking.trackStatusEvent(_Consts.ERROR_TRACK_EVENT, e.message, e.lineNumber + ' @OPEN_WEB_PAGE3');
	                                    });
	                                });
	                            }
	                        };
	                            ...
	                            setTimeout(function () {
	                                _ChromeService2.default.getLastFocusedWindow().then(function (windowDetails) {
	                                    _ChromeService2.default.getCurrentVisibleTab(windowDetails).then(function (tab) {
	                                        ...
	                                        tracking.trackStatusEvent(_Consts.ERROR_TRACK_EVENT, e.message, e.lineNumber + ' @OPEN_WEB_PAGE2');
	                                    });
	                                });
	                                removeTab();
	                            }, delay);
	        ...
	        case ActionTypes.CHANGE_SSE_STATE:
	            retFunc = function retFunc(args) {
	                ...
	                var tracking = _ServiceFactory2.default.get(_Consts.TRACKING_SERVICE);
	                var port = chrome.runtime.connect({ name: 'ws_feature_toggle' });
	        ...
	        case ActionTypes.CHANGE_FOOTER:
	            retFunc = function retFunc(args) {
	                if (args.image && args.image == "tile-bg.png") {
	                    self.setState({
	        ...
	        case ActionTypes.TRIGGER_EVENTS:
	            retFunc = function retFunc(args) {
	                var cond = behaviour.c ? new _Condition2.default(behaviour.c) : null;
	                if (cond === null || cond && cond.check(args)) {
	                    var events = behaviour.events ? behaviour.events : args && args.events ? args.events : false;
	                    ...
	                    for (var event in events) {
	                        if (_typeof(events[event]) == 'object') {
	                            for (var eventName in events[event]) {
	                                ...
	                                _LoggerService2.default.debug('event: ' + eventName);
	                            }
	                        } else {
	            ...
	            retFunc = function retFunc(args) {
	                ...
	                if (cond === null || cond && cond.check(args)) {
	                    ...
	                    if (self.mounted && _typeof(self.state.componentsState) === 'object' && _typeof(args.componentsState) === 'object') {
	                        self.setState(_assign(self.state.componentsState, args.componentsState));
	                    }
	                }
	            };
	            break;
...
/***/ function(module, exports, __webpack_require__) {
	...
	var Addons = __webpack_require__(38);
	var Color = __webpack_require__(70);
	var Display = __webpack_require__(40);
	var GlobalStylesheets = __webpack_require__(43);

	var assign = __webpack_require__(8);
	var createCSS = __webpack_require__(45);
	var curry = __webpack_require__(69);
...
/***/ function(module, exports, __webpack_require__) {
	...
	module.exports = __webpack_require__(40).Flex;
...
/***/ function(module, exports, __webpack_require__) {
	...
	var CSSDisplayNames = __webpack_require__(41);
	var makeStyleComponentClass = __webpack_require__(42);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var explodePseudoStyles = __webpack_require__(44);
	var createCSS = __webpack_require__(45);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var CSSPropertyOperations = __webpack_require__(46);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var CSSProperty = __webpack_require__(47);
	var ExecutionEnvironment = __webpack_require__(48);
	var ReactInstrumentation = __webpack_require__(49);
	...
	var memoizeStringOnly = __webpack_require__(67);
...
/***/ function(module, exports, __webpack_require__) {
	...
	if (process.env.NODE_ENV !== 'production') {
	  var ReactDebugTool = __webpack_require__(50);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var ReactInvalidSetStateWarningDevTool = __webpack_require__(51);
	var ReactHostOperationHistoryDevtool = __webpack_require__(54);
	var ReactComponentTreeDevtool = __webpack_require__(55);
	var ReactChildrenMutationWarningDevtool = __webpack_require__(59);
	var ExecutionEnvironment = __webpack_require__(48);
	...
	var Link = function (_React$Component) {
	    ...
	    _createClass(Link, [{
	        ...
	        value: function render() {
	            (0, _Utils.populateFromLocalStorage)(this.props.config);
	            var prefService = _ServiceFactory2.default.get(_Consts.PREFERENCES_SERVICE);
	            var title = _ChromeService2.default.getTranslationOnlyIfExist(this.props.config.title, prefService.get(_Consts.TRANSLATION_ACTIVE_KEY));
	...
	var Text = function (_React$Component) {
	    ...
	    _createClass(Text, [{
	        ...
	        value: function render() {
	            ...
	            var prefService = _ServiceFactory2.default.get(_Consts.PREFERENCES_SERVICE);
	            var text = this.props.config.dynamic ? this.getCurrentText() : this.props.config.text;
	            if (this.props.config.disable_trans != true) {
	                text = _ChromeService2.default.getTranslationOnlyIfExist(text, prefService.get(_Consts.TRANSLATION_ACTIVE_KEY));
	            }
	            return React.createElement(
	                Block,
	        ...
	        value: function getCurrentText() {
	            /*if(Array.isArray(this.props.config.text_key)) {
	                try {
	                    let text = JSON.parse(localStorage[this.props.config.text_key[0]]);
	                    for (let i = 1; i < this.props.config.text_key.length; i++)
	                        text = text[this.props.config.text_key[i]];
	                    return text;
	                }
	                catch (e){
	                    LoggerService.debug(e.message, e.lineNumber + ' @getCurrentText');
	                }
	            }
	            else*/
	            return localStorage[this.props.config.text_key];
	        }
	    }, {
	        key: "handleMouseEnter",
	        value: function handleMouseEnter(event) {
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _FirstChild = __webpack_require__(74);
	...
	var Item = function (_React$Component) {
	    ...
	    _createClass(Item, [{
	        ...
	        value: function render() {
	            ...
	            if (!this.props.config.hide) {
	                var txt = _ChromeService2.default.getTranslationOnlyIfExist(this.props.config.text, prefService.get(_Consts.TRANSLATION_ACTIVE_KEY));
	...
	var Input = function (_React$Component) {
	    ...
	    _createClass(Input, [{
	        ...
	        value: function render() {
	            ...
	            text = _ChromeService2.default.getTranslationOnlyIfExist(text, prefService.get(_Consts.TRANSLATION_ACTIVE_KEY));
	            var HtmlTag = this.props.config.htmlTag ? this.props.config.htmlTag : 'input';

	            if (!this.props.config.hide) {
	                component = React.createElement(HtmlTag, { type: inputType,
	        ...
	        value: function handleKeyDown(e) {
	            if (this.props.config.events && this.props.config.events.onKeyDown) {
	                ...
	                _LoggerService2.default.debug('event: ' + event.name);
	            }
	        }
	    }]);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var IntervalComponents = function (_React$Component) {
	    ...
	    _createClass(IntervalComponents, [{
	        key: "updateConfigFromCompsState",
	        value: function updateConfigFromCompsState() {
	            if (_typeof(this.state.componentsState) === 'object') {
	                for (var comp in this.state.componentsState) {
	                    if (this.state.componentsState[comp] === true) {
	                        if (this.getComponentIndexById(comp) || this.getComponentIndexById(comp) === 0) this.state.activeComponents[this.getComponentIndexById(comp)].hide = 0;
	                    } else if (this.state.componentsState[comp] === false) {
	                        if (this.getComponentIndexById(comp) || this.getComponentIndexById(comp) === 0) this.state.activeComponents[this.getComponentIndexById(comp)].hide = 1;
	                    }
	                }
	            }
	        }
	        ...
	        value: function updateConfigFromLocalStorage() {
	            ...
	            if (localStorageConfig && (typeof localStorageConfig === "undefined" ? "undefined" : _typeof(localStorageConfig)) === 'object') {
	                for (var each in localStorageConfig) {
	                    if (localStorageConfig[each] === true) {
	                        this.props.config.components[this.getComponentIndexById(each, this.props.config.components)].hide = 0;
	                    } else if (localStorageConfig[each] === false) {
	                        this.props.config.components[this.getComponentIndexById(each, this.props.config.components)].hide = 1;
	                    }
	                }
	            }
	        }
	        ...
	        value: function populateStateArraysFromConfig() {
	            var configForLocalStorage = {};
	            // populate active, inactive, showComponents and the new configForLocalStorage
	            if (this.props.config.components) {
	                this.state.showComponents = [];
	                for (var configComp in this.props.config.components) {
	                    if (this.props.config.components[configComp].active || typeof this.props.config.components[configComp].active === 'undefined') {
	                        this.state.activeComponents.push(this.props.config.components[configComp]);
	                        if (!this.props.config.components[configComp].hide) {
	                            this.state.showComponents.push(this.props.config.components[configComp]);
	                            if (this.props.config.components[configComp].id) {
	                                configForLocalStorage[this.props.config.components[configComp].id] = true;
	                            }
	                        } else if (this.props.config.components[configComp].id) {
	                            configForLocalStorage[this.props.config.components[configComp].id] = false;
	                        }
	                    } else {
	                        this.state.inactiveComponents.push(this.props.config.components[configComp]);
	                    }
	                }
	                if (this.props.config.localStorageKey) {
	                    this.pref.set(this.props.config.localStorageKey, configForLocalStorage);
	                }
	            }
	        }
	    }, {
	        ...
	        value: function startInterval() {
	            var self = this;
	            var interval = self.props.config.interval ? self.props.config.interval : 1000;

	            this.clearInterval();

	            this.interval = setInterval(function () {
	                self.updateConfigFromCompsState();
	        ...
	        value: function renderComponent() {
	            ...
	            return componentKeys.map(function (componentKey) {
	                ...
	                componentConfig = _assign({}, componentConfig, { hide: shouldHide === 1 ? false : shouldHide === 0 ? true : currentComponentKey === componentKey });
	        ...
	        value: function renderButton() {
	            ...
	            return componentKeys.map(function (componentKey) {
	                ...
	                //componentConfig = Object.assign({}, componentConfig, {hide: shouldHide === 1 ? false : shouldHide === 0 ? true : currentComponentKey === componentKey});
	        ...
	        key: "render",
	        value: function render() {
	            if (this.state.showComponents.length === 0) return null;
	        ...
	        key: "getComponentIndexById",
	        value: function getComponentIndexById(compId, compsArray) {
	            if (!compId) return false;
	            var internalArr = compsArray ? compsArray : this.props.config.components;
	            for (var comp in internalArr) {
	                if (internalArr[comp].id === compId) return comp;
	            }
	            return false;
	        }
	    }, {
	        ...
	        value: function handleClick() {
	            this.clearInterval();
	            this.updateConfigFromCompsState();
	            this.populateStateArraysFromConfig();
	            this.setState({
	        ...
	        value: function nextComponentKey() {
	            if (this.state.showComponents.length === 1) return 0; // zero will cause the button to indicate for the widget which already displayed
	            return this.state.currentComponent + 1 === Object.keys(this.state.showComponents).length ? 0 : this.state.currentComponent + 1;
	        }
	    }]);
	...
	var Clock = function (_React$Component) {
	    ...
	    _createClass(Clock, [{
	        ...
	        value: function render() {
	            ...
	            var day = _ChromeService2.default.getTranslationOnlyIfExist(weekday[d.getDay()], prefService.get(_Consts.TRANSLATION_ACTIVE_KEY));
	...
	var Weather = function (_React$Component) {
	    ...
	    _createClass(Weather, [{
	        ...
	        value: function render() {
	            ...
	            var degrees = _ChromeService2.default.getTranslationOnlyIfExist(this.state.isCelsius ? "C" : "F", prefService.get(_Consts.TRANSLATION_ACTIVE_KEY));
	        ...
	        _this.renderHTML = function () {
	            ...
	            var _loop = function _loop(i) {
	                ...
	                components.push(React.createElement(
	                    ...
	                    React.createElement("div", { style: _assign({}, _this3.props.config.imageSeparatorStyle, item.media && item.media.url ? {} : { display: 'none' }) }),
	                    React.createElement(
	                        Block,
	            ...
	            var buttons = React.createElement(
	                ...
	                React.createElement(Container, { onClick: function onClick() {
	                        _this3.toggle(false);_EventsService2.default.trigger(_Consts.RSS_REMOVE_FEED_CLICKED);
	                    }, config: { style: this.props.config.buttonStyle, hover: this.props.config.buttonHoverStyle, components: [{ type: 'Text', text: 'X', style: this.props.config.removeButtonStyle }] } }),
	            ...
	            this.html = [React.createElement(Block, { key: 0, style: this.props.config.feedImageBackgroundStyle }), feed.image && feed.image.url ? React.createElement(
	                ...
	                React.createElement(
	                    ...
	                    React.createElement("img", { style: imageStyle, src: feed.image ? feed.image.url : null, onClick: onLogoClick, title: feed.image.title ? feed.image.title : feed.title })
	                )
	            ) : React.createElement(
	                Block,
	                { key: 1, style: this.props.config.headerStyle },
	                ...
	                React.createElement(
	                    ...
	                    React.createElement(
	                        "span",
	                        { style: this.props.config.feedImageTitleStyle, title: feed.image && feed.image.title ? feed.image.title : feed.title },
	                        feed.image && feed.image.title ? feed.image.title : feed.title
	                    )
	                )
	            ), React.createElement(
	                Block,
	                { key: 2, style: _assign({}, this.props.config.eventsWrapperStyle, { "direction": isRTL(feed.title) ? 'rtl' : 'ltr' }) },
	                components
	            ), React.createElement(
	                Block,
	                ...
	                React.createElement(
	                    "span",
	                    { style: this.props.config.moreStyle, onClick: onMoreClick },
	                    "More"
	                )
	            )];
	        };
	...
	var Image = function (_React$Component) {
	    ...
	    _createClass(Image, [{
	        ...
	        value: function render() {
	            ...
	            var parentStyle = this.props.config.parentStyle || null;
	            ...
	            if (!this.props.config.hide) return React.createElement(
	                "div",
	                { onClick: this.handleClick, style: parentStyle },
	        ...
	        value: function getCurrentImage() {
	            var image = this.state.image && this.state.image.src ? this.state.image.src : this.props.config.image_path && localStorage[this.props.config.image_key] !== '' ? this.props.config.image_path + localStorage[this.props.config.image_key] : localStorage[this.props.config.image_key];
	            if (this.props.config.fallback_image && image === '') {
