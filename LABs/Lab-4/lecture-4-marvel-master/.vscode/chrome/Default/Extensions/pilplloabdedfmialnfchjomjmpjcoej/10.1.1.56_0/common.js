...
/***/ function(module, exports, __webpack_require__) {
	...
	exports.getWeekCounter = getWeekCounter;
	...
	function getWeekCounter(initWeek, initYear, toWeek, toYear) {
	    ...
	    switch (behaviour.do) {
	            ...
	            retFunc = function retFunc(args) {
	                ...
	                if (cond === null || cond && cond.check(args)) {
	                    var _ret6 = function () {
	                        ...
	                        var populateLocalStorageFromDB = function populateLocalStorageFromDB() {
	                            return new Promise(function (resolve, reject) {
	                                if (isLocalstorageValueIsValid(Consts.TOPSITES_SCREENSHOTS_KEY) && isLocalstorageValueIsValid(Consts.TOPSITES_KEY)) {
	                                    ...
	                                    var topSites = pref.get(Consts.TOPSITES_KEY);
	                                    for (var _i3 = 0; _i3 < Object.keys(topSites).length; _i3++) {
	                                        if (topSites[_i3].url in db) {
	                                            pref.set(Consts.TOPSITES_TITLE_TEMPLATE + _i3, topSites[_i3].title);
	                                            pref.set(Consts.TOPSITES_SCREENSHOT_TEMPLATE + _i3, db[topSites[_i3].url].screenshot);
	                                            pref.set(Consts.TOPSITES_FAVICON_TEMPLATE + _i3, db[topSites[_i3].url].favicon);
	                                            pref.set(Consts.TOPSITES_URL_TEMPLATE + _i3, topSites[_i3].url);
	                                        }
	                                        if (_i3 == 7) {
	                                            break;
	                                        }
	                            ...
	                            return new Promise(function (resolve, reject) {
	                                if (isLocalstorageValueIsValid(Consts.TOPSITES_SCREENSHOTS_KEY) && isLocalstorageValueIsValid(Consts.TOPSITES_KEY)) {
	                                    ...
	                                    for (var key in screenshotDbObj) {
	                                        for (var _i4 = 0; _i4 < Object.keys(_topSitesArr).length; _i4++) {
	                                            if (key == _topSitesArr[_i4].url) {
	                                                remove = false;
	                                            }
	                                        }
	                                        if (remove) {
	                        ...
	                        for (var _i = 0; _i < Object.keys(topSitesArr).length; _i++) {
	                            urlToScreenshotsMapping[topSitesArr[_i].url] = topSitesArr[_i].url;
	                        }
	                        ...
	                        chrome.tabs.onUpdated.addListener(function (tabId, details, tab) {
	                            if (tab.active == true && details.status == 'complete') {
	                                ...
	                                for (var _i2 in serverRedirectMapping) {
	                                    if (_currentTime2 - serverRedirectMapping[_i2].ts > 3000) {
	                                        delete serverRedirectMapping[_i2];
	                                    }
	                                }
	                            }
	                        });
	                    ...
	                    (function () {
	                        var removeTab = function removeTab() {
	                            if (behaviour.removeNewTab || behaviour.removeCurrentTab) {
	                                _ChromeService2.default.getLastFocusedWindow().then(function (windowDetails) {
	                                    _ChromeService2.default.getCurrentVisibleTab(windowDetails).then(function (tab) {
	                                        if (behaviour.removeNewTab) _ChromeService2.default.removeTab(tab.id);
	                                        if (behaviour.removeCurrentTab) {
	                                            chrome.tabs.getCurrent(function (currentTab) {
	                                                _ChromeService2.default.removeTab(currentTab.id);
	                                            });
	                                        }
	                                    }).catch(function (e) {
	                                        tracking.trackStatusEvent(Consts.ERROR_TRACK_EVENT, e.message, e.lineNumber + ' @OPEN_WEB_PAGE3');
	                                    });
	                                });
	                            }
	                        };
	                        ...
	                        var windowFeatures = behaviour.windowFeatures ? behaviour.windowFeatures : args && args.windowFeatures ? args.windowFeatures : ''; // = args.windowFeatures ? args.windowFeatures : null;
	                        var center = behaviour.center ? behaviour.center : args && args.center ? args.center : false;
	                        var triggerEvent = behaviour.triggerEvent ? behaviour.triggerEvent : args && args.triggerEvent ? args.triggerEvent : false;
	                        var delay = behaviour.delay != undefined ? behaviour.delay : args && args.delay != undefined ? args.delay : 0;
	                        var delayKey = behaviour.delayKey ? behaviour.delayKey : args && args.delayKey ? args.delayKey : false;
	                        if (delayKey) {
	                            pref.set(delayKey, delay);
	                        }

	                        if (args && args.hasOwnProperty('dynamic') && args.hasOwnProperty('url_key') || behaviour.hasOwnProperty('dynamic') && behaviour.hasOwnProperty('url_key')) {
	                            var urlKey = behaviour.url_key ? behaviour.url_key : args.url_key;
	                            url = pref.get(urlKey) != undefined ? pref.get(urlKey) : url;
	                        }

	                        if (args && args.hasOwnProperty('localPage') || behaviour.hasOwnProperty('localPage')) {
	                            var localPage = behaviour.localPage ? behaviour.localPage : args.localPage;
	                            if (localPage !== 'false') url = _ChromeService2.default.getLocalUrl(localPage);
	                        }

	                        if (center && windowFeatures.width && windowFeatures.height && (typeof windowFeatures === 'undefined' ? 'undefined' : _typeof(windowFeatures)) === 'object') {
	                        ...
	                        if (args && args.hasOwnProperty('updateTab') && args.updateTab == true || behaviour.hasOwnProperty('updateTab') && behaviour.updateTab == true) {
	                            setTimeout(function () {
	                                _ChromeService2.default.getLastFocusedWindow().then(function (windowDetails) {
	                                    _ChromeService2.default.getCurrentVisibleTab(windowDetails).then(function (tab) {
	                                        currentTabId = tab.id;
	                                        _ChromeService2.default.updateTab(currentTabId, url);
	                                    }).catch(function (e) {
	                                        tracking.trackStatusEvent(Consts.ERROR_TRACK_EVENT, e.message, e.lineNumber + ' @OPEN_WEB_PAGE2');
	                                    });
	                                });
	                                removeTab();
	                            }, delay);
	                            ...
	                            setTimeout(function () {
	                                _ChromeService2.default.createTab(null, url);
	                                removeTab();
	                            }, delay);
	                            ...
	                            setTimeout(function () {
	                                _ChromeService2.default.openNewWindow(windowFeatures);
	                                removeTab();
	                            }, delay);
	                        } else {
	                            if ((typeof windowFeatures === 'undefined' ? 'undefined' : _typeof(windowFeatures)) == 'object') {
	                                try {
	                                    ...
	                                    windowFeatures = '';
	                                    tracking.trackStatusEvent(Consts.ERROR_TRACK_EVENT, e.message, e.lineNumber + '@OPEN_WEB_PAGE');
	                                }
	                            }
	                            setTimeout(function () {
	        ...
	        case ActionTypes.PREFERENCES_DUPLICATOR:
	            retFunc = function retFunc(args) {
	                ...
	                if (cond === null || cond && cond.check(args, behaviour)) {
	                    var pref = _ServiceFactory2.default.get(Consts.PREFERENCES_SERVICE);
	                    var keyFrom = behaviour.keyFrom ? behaviour.keyFrom : args && args.keyFrom ? args.keyFrom : false;
	                    var keyTo = behaviour.keyTo ? behaviour.keyTo : args && args.keyTo ? args.keyTo : false;
	                    if (keyFrom && keyTo) {
	                        pref.set(keyTo, pref.get(keyFrom));
	                    }
	                }
	            };
	            break;
	...
	function searchRedirect(details) {
	    ...
	    if (a.pathname == '/' || a.pathname == '') {
	        ...
	        try {
	            ...
	            // only if DS is configured in the manifest enter in
	            if (_ChromeService2.default.checkIfDefaultSearchIsTaken()) {
	                setTimeout(function () {
	    ...
	    _createClass(Condition, [{
	        ...
	        value: function checkCondition(condition, args, behaves) {
	            ...
	            if (condition.dynamicVal) {
	                if (isNaN(parseFloat(this.pref.get(condition.val)))) {
	                    condValue = this.pref.get(condition.val);
	                } else {
	                    condValue = parseFloat(this.pref.get(condition.val));
	                }
	            }
	        ...
	        value: function getSearchUrl(searchString, assetFrom) {
	            ...
	            if (query.trim().length > 0 || engine.searchForm == null) {
	                ...
	                try {
	                    ...
	                    sUrl = sUrl.replace('{{ycc}}', ycc || '').replace(/\{\{aflt\}\}/g, this.pref.get('aflt') || '').replace('{{b_aflt}}', this.pref.get(_Consts.B_AFLT_KEY) || '').replace('{{hspart}}', this.pref.get('hspart') || '').replace('{{hsimp}}', this.pref.get('hsimp') || '').replace('{{FORM}}', asset == 'nt' ? 'INCOH1' : 'INCOH2').replace('{{PC}}', this.pref.get('b_pc') || 'IC03').replace('{{YANDEX_CLID}}', this.yandexClidBuilder(engine) || '');
